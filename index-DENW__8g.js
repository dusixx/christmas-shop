...
var { header, siteNav, burgerBtn, burgerMenu } = {
	header: document.querySelector(`.${cls$5.header}`),
	siteNav: document.querySelector(`.${cls$5.siteNav}`),
	burgerBtn: document.querySelector(`.${cls$5.burgerBtn}`),
	burgerMenu: document.querySelector(`.${cls$5.burgerMenu}`)
};
...
new class BurgerMenu {
	...
	constructor(opts) {
		...
		const menuItem = burgerMenu.querySelectorAll(`.${cls$5.siteNavItem}`);
	...
	#toggleMenu = () => {
		...
		burgerBtn.classList.toggle(cls$5.burgerBtnActive);
		return burgerMenu.classList.toggle(cls$5.burgerMenuActive);
	};
	toggle() {
...
var backdrop = document.querySelector(`.${cls$4.backdrop}`);
var modal$1 = backdrop?.querySelector(`.${cls$4.modal}`);
var refs$1 = {
	root,
	backdrop,
	modal: modal$1,
	modalContent: modal$1?.querySelector(`.${cls$4.modalContent}`),
	modalCloseBtn: modal$1?.querySelector(`.${cls$4.modalCloseBtn}`)
};
//#endregion
...
var Backdrop = class Backdrop {
	...
	toggle() {
		Scroll.toggleLock();
		const wasShown = this.ref.classList.toggle(cls$4.backdropActive);
		if (wasShown) {
...
var Modal = class Modal {
	...
	constructor({ hideOnBackdropClick: hideOnClick, hideOnEscape } = {}) {
		...
		this.#ref = refs$1.modal;
		this.#contentRef = refs$1.modalContent;
		refs$1.modalCloseBtn.addEventListener("click", () => this.#backdrop.toggle());
	}
	show({ content } = {}) {
	...
	set content(markup) {
		...
		this.ref.style.top = refs$1.root.clientHeight <= this.height ? TOP_OFFSET : null;
...
var makeSuperpowerIcons = (points) => {
	...
	const markup = Array.from({ length: totalCount }, (itm) => {
		return ` 
        <li class="${cls$3.giftSuperpowers}__icon">
            <svg><use href="./images/icons.svg#icon-snowflake" ${--activeCount >= 0 ? activeStyle : ``}></use></svg>
        </li>
    `;
	}).join("");
	return `<ul class=${cls$3.giftSuperpowers}__icons>${markup}</ul>`;
};
var makeSuperpowerList = (superpowers) => {
	const markup = Object.entries(superpowers).map(([name, points]) => {
		return `
        <li class="${cls$3.giftSuperpowers}__item">
            <p class="${cls$3.giftSuperpowers}__name">${name}</p>
            <div class="${cls$3.giftSuperpowers}__value">
                <p class="${cls$3.giftSuperpowers}__points">${points}</p>
                ${makeSuperpowerIcons(points)}
            </div>
        </li>
      `;
	}).join("");
	return `<ul class=${cls$3.giftSuperpowers}__list>${markup}</ul>`;
};
var makeGiftCard = ({ name, category, description, superpowers } = {}, interactive = true, extraClasses = "") => {
	const classList = joinClasses(cls$3.giftCard, interactive && `${cls$3.giftCard}--interactive`, category && `${cls$3.giftCard}--${makeId(category)}`, ...extraClasses.split(/\s+/));
	const desc = description ? `<p class="${cls$3.giftCard}__description">${description}</p>` : ``;
	const superpow = superpowers ? `
        <div class="${cls$3.giftSuperpowers}">
            <h4 class=${cls$3.giftSuperpowers}__title>Adds superpowers to:</h4>
            ${makeSuperpowerList(superpowers)}
        </div>` : ``;
	return `
    <article class="${classList}" id="${makeId(name)}">
        <div class="${cls$3.giftCard}__thumb"></div>
        <div class="${cls$3.giftCard}__desc">
            <div class="${cls$3.giftCard}__heading">
                <h4 class="${cls$3.giftCard}__tag">${category}</h4>
                <h3 class="${cls$3.giftCard}__title">${name}</h3>
                ${desc}
            </div>
            ${superpow}
        </div>
...
var makeDetailedGiftCard = (cardData) => {
	return makeGiftCard(cardData, false, cls$3.giftCardDetailed);
};
var makeGiftList = (items) => {
	elementExpected(items, "array");
	const markup = items.map(({ name, category }) => `<li class="${cls$3.giftList}__item">${makeGiftCard({
		name,
		category
	})}</li>`).join("");
	return `<ul class="${cls$3.giftList}">${markup}</ul>`;
};
//#endregion
...
var GiftList = class GiftList {
	...
	constructor(opts) {
		...
		this.#container = document.querySelector(`.${cls$2.giftsContainer}`);
		this.onClick = opts?.onClick;
	}
	#handleGiftListClick({ target }) {
		if (!isFunc(this.#onClick)) return;
		const targetCard = target.closest(`.${cls$2.giftCard}`);
	...
	render() {
		...
		this.#ref = this.#container.querySelector(`.${cls$2.giftList}`);
